  ...
  const exportToExcel = () => {
    const data = items.flatMap((item) =>
      item.events.map((event, index) => ({
        [t('sharedDevice')]: index ? '' : devices[item.deviceId].name,
        [t('positionFixTime')]: formatTime(event.eventTime, 'seconds'),
        [t('sharedType')]: t(prefixString('event', event.type)),
      }))
    );
  ...
  const exportToPDF = async () => {
    try {
      ...
      try {
        ...
        doc.setFont('Helvetica', 'normal'); // فونت پیش‌فرض در صورت خطا
      }

      doc.setFontSize(12);

      // اضافه کردن عنوان گزارش
      doc.text(t('reportCombined'), 14, 10);
      ...
      // ایجاد جدول در PDF
      autoTable(doc, {
        head: [[t('sharedDevice'), t('positionFixTime'), t('sharedType')]],
        body: tableData,
        styles: { font: doc.getFont().fontName, halign: 'right', fontStyle: 'bold' },
        headStyles: { fillColor: [41, 128, 185] },
      });

      doc.save('CombinedReport.pdf');
    } catch (error) {
