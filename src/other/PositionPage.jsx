  ...
  useEffectAsync(async () => {
    if (id) {
      ...
      if (positions.length > 0) {
        // کم کردن یک روز از ویژگی‌های زمانی در شیء position
        const modifiedPosition = {
          ...positions[0],
          fixTime: positions[0].fixTime ? new Date(new Date(positions[0].fixTime).getTime() - 24 * 60 * 60 * 1000).toISOString() : positions[0].fixTime,
          deviceTime: positions[0].deviceTime ? new Date(new Date(positions[0].deviceTime).getTime() - 24 * 60 * 60 * 1000).toISOString() : positions[0].deviceTime,
          serverTime: positions[0].serverTime ? new Date(new Date(positions[0].serverTime).getTime() - 24 * 60 * 60 * 1000).toISOString() : positions[0].serverTime,
        };
        setItem(modifiedPosition);
      }
    }
  }, [id]);
