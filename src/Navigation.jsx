...
const Navigation = () => {
  ...
  // --- پردازش پارامترهای query / token / deviceId و غیره
  useEffectAsync(async () => {
    try {
      ...
      // در صورت نیاز خطا را هندل کن (لاگ یا نمایش پیغام)
      setRedirectsHandled(true);
    }
  }, [query]);

  if (!redirectsHandled) return <Loader />;

  // --- وضعیت لندینگ (خواندن synchronous از localStorage)
const isLandingShown = sessionStorage.getItem("landingShown") === "true";

  // ===== حالت 1: اگر لندینگ هنوز دیده نشده -> فقط روت‌های landing/login/... فعال باشند
  if (!isLandingShown) {
    return (
      <Routes>
        <Route path="/landing" element={<Landing />} />

        {/* صفحات auth & server که باید قبل/حین لندینگ هم در دسترس باشند */}
        <Route path="/login" element={<LoginPage />} />
        <Route path="/register" element={<RegisterPage />} />
        <Route path="/reset-password" element={<ResetPasswordPage />} />
        <Route path="/change-server" element={<ChangeServerPage />} />

        {/* اگر کاربر هر مسیر دیگری زد، هدایت به لندینگ */}
        <Route path="*" element={<Navigate to="/landing" replace />} />
      </Routes>
    );
  }

  // ===== حالت 2: لندینگ قبلاً دیده شده -> رندر کامل روت‌های پروژه
  return (
    <Routes>
      {/* (قابلیت بازدید مجدد لندینگ در صورت نیاز) */}
      <Route path="/landing" element={<Landing />} />

      {/* صفحات auth در دسترس بمانند */}
      <Route path="/login" element={<LoginPage />} />
      <Route path="/register" element={<RegisterPage />} />
      <Route path="/reset-password" element={<ResetPasswordPage />} />
      <Route path="/change-server" element={<ChangeServerPage />} />

      {/* روت اصلی پروژه با تمام روت‌های داخلی */}
      ...
      {/* هر مسیر غیرمجاز => می‌تونی به / یا 404 هدایت کنی */}
