...
import { startOfDay, addWeeks, format } from 'date-fns-jalali';
...
const PreferencesPage = () => {
  ...
  const [tokenExpiration, setTokenExpiration] = useState(addWeeks(startOfDay(new Date()), 1));
  ...
  const generateToken = useCatch(async () => {
    const expiration = format(tokenExpiration, 'yyyy-MM-dd');
